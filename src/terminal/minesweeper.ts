...
export async function minesweeperCommand(ctx: CommandContext): Promise<void> {
  ...
  const keyHandler: KeyHandler = (
    ...
    // Navigation
    if ((key === "ArrowUp" || key.toLowerCase() === "w") && !state.gameOver) {
      state.cursorY = Math.max(0, state.cursorY - 1);
    }
    if ((key === "ArrowDown" || key.toLowerCase() === "s") && !state.gameOver) {
      state.cursorY = Math.min(state.height - 1, state.cursorY + 1);
    }
    if ((key === "ArrowLeft" || key.toLowerCase() === "a") && !state.gameOver) {
    ...
    // Reveal cell
    if ((key === " " || key === "Enter") && !state.gameOver) {
      if (state.firstMove) {
    ...
    // Flag cell
    if (key.toLowerCase() === "f" && !state.gameOver) {
      if (!state.firstMove) {
